...
async function run() {
  ...
  console.log('--- MEASURING OPTIMIZED AFTER FLOW ---')
  ...
  // 3. Test Optimized Image Upload (Realistic compressed luxury product image WebP ~200-300KB)
  const heroPath = path.join(__dirname, '..', 'storefront', 'public', 'Hero.webp')
  const optimizedBuffer = fs.readFileSync(heroPath)
  console.log(`[PERF] optimized image file size: ${(optimizedBuffer.length / 1024).toFixed(1)} KB`)

  const tUploadStart = performance.now()
  const formData = new FormData()
  const blob = new Blob([optimizedBuffer], { type: 'image/webp' })
  formData.append('image', blob, 'optimized-product.webp')
  ...
  console.log(`[PERF] optimized image upload duration: ${uploadDuration.toFixed(1)} ms (${(uploadDuration / 1000).toFixed(2)} s)`)
  console.log(`[PERF] uploaded image url: ${uploadJson.url}`)

  // 4. Test Product Creation with Variants (measuring optimized parallel lookups)
  const tProductStart = performance.now()
  const payload = {
    name: `Perf Test Product After ${Date.now()}`,
  ...
  console.log('--- SUMMARY OF OPTIMIZED (AFTER) ---')
